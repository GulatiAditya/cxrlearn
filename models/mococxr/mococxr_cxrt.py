...
__MODELS = {"resnet18": "./pretrained/moco-cxr_resnet18.pth.tar",
            "resnet50": "./pretrained/moco-cxr_resnet50.pth.tar"}
...
def _mococxr(model="resnet50", device=torch.device("cpu"), freeze_backbone=True, num_out=None):
    """
    ...
    assert isinstance(
        device, torch.device), "device has to be a type torch.device, use torch.device('"'cuda:0'"' if torch.cuda.is_available() else '"'cpu'"') before calling mococxr function"
